...
- 解决问题：原认购弹窗快捷金额写死为 `1元/10元/50元/全包`，当后台配置的参与人最低认购金额不是 1 元时，第一项不符合真实认购规则；用户也只能点加减或手输金额，不能通过拖动快速调整。
- 实施内容：
  1. `useGroupBuyDetail` 新增动态快捷认购金额、滑块最小值、最大值、步进、禁用状态和展示金额；第一项固定使用当前有效最低认购金额。
  2. 快捷按钮、手动输入、加减按钮和滑块统一调用现有 `normalizeJoinAmount`，确保金额按单份金额取整，并自动处理尾单全包。
  3. `GroupBuyView` 认购弹窗新增 `van-slider`，拖动时同步金额输入框和底部共计金额；快捷按钮改为渲染动态选项。
  4. 同步更新 `架构设计.md` 和前端组件规范，沉淀手机端合买认购金额控件规则。
- 验证结果：手机端 `npm run build` 通过，`npm test` 通过但当前测试套件为空，`git diff --check` 通过。
...
- 解决问题：原阶段性补单按固定累计目标推进，容易出现每个阶段补单金额相同；调度重复进入同一阶段时也可能继续追加补单，和“每阶段不要超过配置百分比，最后再补满”的运营预期不一致。
- 实施内容：
  1. 后端 `RobotConfigSummary` 新增 `groupBuyRhythmFillMaxPercent` 字段，默认 20，校验范围 1-100，并新增数据库迁移 `20260626172000_add_group_buy_rhythm_fill_max_percent.sql` 持久化该配置。
  2. `rhythm` 策略改为前三个阶段每阶段最多补合买总金额的配置百分比；同一计划同一阶段只补一次，非最终阶段如果会直接满单则等待最终阶段，最后 15 秒仍直接补满剩余金额并生成真实合买订单。
  3. 阶段性多用户拆分改为金额充足时按稳定随机权重拆分，避免同一阶段多条机器人参与记录金额完全相同；金额不足时继续遵守最低认购额和最低份额约束。
  4. 发单机器人初始种子认购只允许执行一次，避免后续重复扫描已有计划时持续追加种子认购并挤占阶段补单空间。
  5. 管理后台补单机器人表单新增“阶段单次最高百分比”输入框，列表策略文案展示当前百分比上限；同步更新 `架构设计.md`、`.trellis/spec/backend/api-contracts.md` 和 OpenAPI 摘要。
- 验证结果：`cargo test --manifest-path backend/Cargo.toml group_buy_robot -- --nocapture` 17 个相关测试通过；`cargo test --manifest-path backend/Cargo.toml robot_repository -- --nocapture` 5 个相关测试通过；`cd admin && npm run build` 通过。检查输出仍有既有后端 unused 警告和前端 chunk size 警告，本次未新增失败。
...
- 解决问题：
  1. 发单机器人创建合买计划时只有发起人（U90001）认购，补单机器人（Purchase 类型）只在封盘前 90 秒窗口内才认购，导致合买大厅在大部分时间里只有一个人参与。
  2. DB 模式下 `X90002`-`X90010` 机器人补单用户不在数据库 `users` 表中（只在内存种子数据中有），`add_participant` 调用 `find_user` 时返回 `NotFound`，导致补单失败、期号暂缓开奖。
- 实施内容：
  1. 新增 `seed_robot_plan_initial_participants`：发单机器人创建计划后立即用 `X90002`-`X90010` 多个机器人用户认购剩余金额的 1/3，让合买大厅从一开始就有多个参与者。
  2. `load_access_store`（DB 模式）初始化时自动检查并补齐缺失的 `X90002`-`X90010` 机器人补单用户到数据库 `users` 表。
  3. 更新受影响的测试断言以适配种子认购行为。
...
- 解决问题：`current_open_issue` 用 `scheduled_at >= now` 过滤期号（计划开奖时间晚于当前时间），但调度器 tick 可能在期号已到开奖时间但尚未开奖时运行，此时 `scheduled_at < now` 导致期号被排除，机器人发单被跳过，合买大厅缺少该彩种的机器人合买。
- 实施内容：
  1. `current_open_issue` 过滤条件从 `scheduled_at >= now` 改为 `sale_closed_at > now`（封盘时间还没到才是可销售期号），确保所有仍在销售中的 Open 期号都能被机器人选中发单。
  2. 配合之前的 fallback 覆盖逻辑（未覆盖彩种用默认机器人补发），保证每个开启合买的彩种都有机器人合买。
- 验证结果：`cargo fmt --check` 通过、全量 424 个测试通过。
...
- 解决问题：`run_group_buy_robots` 原来只按机器人的 `lottery_ids` 配置生成任务，如果某些彩种未被任何机器人配置，合买大厅会缺失机器人发布的合买计划。改为任务收集完成后遍历所有启用合买的彩种，未被覆盖的彩种自动使用首个可用的发单机器人补发。
- 实施内容：
  1. 将 `robots.list().await?` 提前保存为 `all_robots`，循环改为 `for robot in &all_robots`。
  2. 循环结束后收集已被发单机器人覆盖的彩种 ID 集合。
  3. 遍历所有启用合买的彩种，对未覆盖的彩种使用首个启用的发单机器人补建任务。
  4. 导入 `BTreeSet` 支持彩种覆盖检测。
- 验证结果：`cargo fmt --check` 通过、`cargo check` 编译通过、全量 423 个测试通过。
...
- 解决问题：不常用模块数量较多时会拉长侧边栏，日常进入后台处理客服、财务、订单和控奖时需要额外扫描低频入口。
- 实施内容：`AppShell` 为导航分组增加展开状态，“常用”默认展开，“不常用”默认收起；分组标题改为可点击按钮并显示展开/收起图标；如果刷新恢复到“不常用”里的某个页面，会自动展开该分组展示当前选中入口。
- 验证结果：`pnpm --dir admin build` 和 `git diff --check` 均通过；管理后台构建仍保留既有大 chunk 提示。
...
- 实施内容：GitHub Actions 删除 `latest` 发布规则，`main` 分支只发布 `sha-<提交短哈希>`，推送 `v*` Git tag 时额外发布同名版本镜像；`docker/metadata-action` 显式设置 `flavor: latest=false`，避免 tag 发布时自动补出 `latest`；新增 `docker-compose.ghcr.yml`，要求服务器通过 `BC_IMAGE_TAG` 显式指定镜像版本；本地 Compose 镜像标签改为 `bc-platform:local`；同步更新 `部署说明.md`、`架构设计.md` 和 Trellis 容器部署规范。
- 验证结果：`.github/workflows/ci.yml`、`docker-compose.yml` 和 `docker-compose.ghcr.yml` 的 YAML 解析通过；`BC_IMAGE_TAG=sha-fca561e DATABASE_URL=postgresql://user:password@127.0.0.1:5432/bc docker compose -f docker-compose.ghcr.yml config` 渲染出明确 GHCR 镜像标签；`docker compose config` 渲染本地 Compose 标签为 `bc-platform:local`；`git diff --check` 通过；第一次 `v2026.06.23-1` 发布暴露 metadata-action 自动补 `latest`，已通过 `latest=false` 修正并准备重新发布干净版本。

## 2026-06-23 11:09 HKT 后台侧边栏常用分组调整

- 完成任务：将后台侧边栏中的“计奖派奖”和“邀请管理”移入“不常用”分组。
- 解决问题：这两个模块不属于日常高频入口，继续放在“常用”会挤占在线客服、财务、用户、合买、订单、彩种控制台等核心运营入口的首屏空间。
- 实施内容：调整 `admin/src/App.tsx` 的 `COMMON_NAVIGATION_ORDER`，移除 `settlements` 和 `invite`，让它们沿用现有不常用分组编号规则自动展示。
- 验证结果：`pnpm --dir admin build` 和 `git diff --check` 均通过；管理后台构建仍保留既有大 chunk 提示。
...
- 解决问题：上一版仍是“入口条 + 多彩种列表/抽屉”的思路，不符合“点击顶部彩种后，下方只显示该彩种控制和数据”的页面结构。
- 实施内容：`LotteryConsolePage.tsx` 以 `saleEnabled=true` 且 `drawControlEnabled=true` 的彩种生成顶部 Tab；默认自动选中第一个可控销售彩种；点击 Tab 后刷新并切换当前彩种；把原“控制开奖号码”抽屉内容改为页内详情面板，集中展示封盘倒计时、开奖倒计时、当前期号、最近开奖、总开关、控制范围、控制期号/订单、开奖号码、用户下单信息和合买发起记录；删除旧多彩种卡片展示口径；架构说明和前端组件规范同步改为当前布局口径。
- 验证结果：`pnpm --dir admin build` 和 `git diff --check` 均通过。

## 2026-06-22 19:47 HKT 彩种控制台可控彩种入口排版修正

- 完成任务：修正彩种控制台可控彩种入口区的排版实现，不再照搬手绘图黑框大按钮样式。
- 解决问题：上一版把图示当作样式参考，使用了过大的黑色描边按钮，导致一排展示密度不足且末尾彩种单独换行；当前需求是优化排版，不要求外观照图。
- 实施内容：`LotteryConsoleControlLotteryStrip` 保留横向入口区结构，但收紧容器内边距、按钮高度、最小宽度和间距，按钮改回后台轻量入口样式，使同宽度下能一行展示更多彩种；架构说明同步改为“轻量入口条”口径。
- 验证结果：`pnpm --dir admin build` 和 `git diff --check` 均通过。

## 2026-06-22 19:42 HKT 彩种控制台状态筛选改为可控彩种入口

- 完成任务：移除后台彩种控制台“状态筛选”区域，改为可控制销售彩种入口区。
- 解决问题：旧筛选区包含状态按钮和名称搜索，但当前控奖场景只需要看到“允许控制且销售中”的彩种，状态筛选会干扰运营直接进入控奖。
- 实施内容：`LotteryConsolePage.tsx` 删除状态筛选类型、状态筛选 state、名称搜索 state、筛选组件和旧匹配函数；新增 `LotteryConsoleControlLotteryStrip`，以横向入口条展示 `saleEnabled=true` 且 `drawControlEnabled=true` 的彩种；按钮点击直接打开“控制开奖号码”抽屉；下方彩种卡片列表同样只展示这批彩种；架构说明同步更新展示口径。
- 验证结果：`pnpm --dir admin build` 和 `git diff --check` 均通过。

## 2026-06-22 19:25 HKT 彩种控制台统计卡片网格移除

- 完成任务：移除后台彩种控制台顶部 `grid gap-3 sm:grid-cols-2 xl:grid-cols-4` 指标卡片区。
- 解决问题：彩种控制台进入页面后先展示“彩种总数、销售开启、开盘中、待开奖”等统计卡片，占用页面空间；当前要求该区域不要显示，让运营更快看到筛选和具体彩种。
- 实施内容：删除 `LotteryConsolePage.tsx` 中 `metrics` 计算、`MetricCard` 引用和对应 `<section>`；页面标题和错误提示后直接展示后续维护的彩种入口区与彩种卡片列表；架构说明同步记录控制台不再保留顶部统计卡片区。
- 验证结果：`pnpm --dir admin build` 和 `git diff --check` 均通过。

## 2026-06-22 19:18 HKT 聊天大厅用户名前端带星号兜底

- 完成任务：修复手机端聊天大厅用户名看起来仍未脱敏的问题。
- 解决问题：上一轮后端公开出口和手机端兜底只返回可见前缀，不补星号；如果接口已经把原始用户名截断，前端继续按截断值展示，就会让用户看到“没有脱敏效果”。
- 实施内容：后端聊天大厅公开用户名恢复为“可见前缀 + `*` 替代剩余字符”；手机端 `ChatHallView.vue` 的 `maskedChatHallUsername` 增加前端最终兜底，已带星号的值直接展示，未带星号且长度达到 4 位的值强制补星号；消息头、头像 `alt` 和红包领取记录继续复用同一函数；架构说明同步更新展示规则。
- 验证结果：`cargo test --manifest-path backend/Cargo.toml public_chat_hall -- --nocapture`、`pnpm --dir mobile build` 和 `git diff --check` 均通过。
...
- 解决问题：充值赠送新增空白档位后会被草稿解析逻辑立刻过滤，导致点击“新增档位”看起来没有效果；代理返利实际策略在“返利管理 / 策略配置”，但系统设置中缺少明确入口，容易被误认为配置丢失。
- 实施内容：充值赠送档位草稿解析保留空白新增行，保存时仍按元金额校验并转为分；系统设置的“返利设置”固定展示“代理返利配置入口”，点击后直接打开“返利管理 / 策略配置”；隐藏历史兼容字段 `recharge_rebate_mode` 的通用编辑入口，避免误保存无效配置；同步更新架构说明。
...
- 解决问题：部分平台彩种即使业务上应按自然节点开奖，数据库里仍可能保留 `schedule.periodic`；旧逻辑会把 `now` 或上一期 `00:05:30` 这类偏移时间作为下一轮基线，导致后续生成 `00:10:30`、`00:15:30`，看起来开奖时间越来越慢。
- 实施内容：当彩种为平台开奖、普通周期且周期能整除一天时，期号生成先按当天 `00:00:00` 自然时钟节点对齐，再计算下一期开奖时间；历史偏移期号只用于定位当前节点，不再继续传递秒级偏移；新增期号生成和调度器测试覆盖调度晚跑、历史偏移期号重对齐和调度生成未来期号。
- 验证结果：`cargo fmt --manifest-path backend/Cargo.toml`、`cargo check --manifest-path backend/Cargo.toml`、后端全量 `cargo test --manifest-path backend/Cargo.toml`（376 个测试成功）均通过。
...
- 解决问题：角色权限细粒度改造时把 `admin_roles.permissions` 字段写回了已发布的 `20260603152000_create_business_tables.sql`，并同步改动了已发布的 `20260603234000_add_all_column_comments.sql`，导致已经执行过旧版本迁移的数据库 checksum 与当前代码不一致，Docker 或本地启动都会在 SQLx migration 阶段退出。
- 实施内容：恢复 `20260603152000_create_business_tables.sql` 中 `admin_roles` 的原始结构；删除 `20260603234000_add_all_column_comments.sql` 中提前引用 `admin_roles.permissions` 的字段注释；保留 `20260619143000_add_admin_role_permissions.sql` 作为唯一创建和注释 `permissions` 字段的前向迁移；后端迁移入口新增受控修复逻辑，如果遇到 `20260603152000` 或 `20260603234000` 的 checksum 冲突，会确认 `admin_roles` 表存在后更新 `_sqlx_migrations` 的对应版本 checksum，并循环继续执行后续迁移。
- 验证结果：`cargo fmt --manifest-path backend/Cargo.toml`、`cargo fmt --manifest-path backend/Cargo.toml --check`、`cargo check --manifest-path backend/Cargo.toml` 和 `git diff --check` 均通过；同时已比对确认 `20260603152000_create_business_tables.sql` 与权限改造前发布版本一致，`permissions` 字段只由 `20260619143000_add_admin_role_permissions.sql` 创建。
...
# 2026-06-23 21:03 HKT 手机端我的记录 Tab 顺序调整

- 完成任务：把手机端 `/orders` 页面里的“我的合买”和“我的注单”位置对调。
- 解决问题：原来“我的注单”在左、“我的合买”在右，不符合当前用户希望优先看到合买入口的页面顺序。
- 实施内容：调整 `HistoryView` 中订单记录 Tab 配置顺序，把“我的合买”放在前面，“我的注单”放在后面；接口、分页、默认选中视图和列表数据逻辑保持不变。
- 验证结果：`pnpm --dir mobile build` 和 `git diff --check` 均通过。
