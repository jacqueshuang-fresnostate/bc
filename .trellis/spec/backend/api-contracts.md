...
- 范围：`GET /api/user/invitations/summary`、`AccessRepository` 用户代理关系、`InviteRepository` 邀请记录、`FinanceRepository` 充值与返利流水、`WithdrawalRepository` 已通过提现记录、手机端 `mobile/src/api/user.ts`。
