...
- 彩种对象必须返回 `saleCloseLeadSeconds`，默认 `1`，表示本期开盘后可售多少秒；字段名继续沿用是为了兼容已发布接口。
- 平台开奖生成期号时按 `issueFormat` 渲染计划开奖时间；`{seq1}`、`{seq2}`、`{seq3}`、`{seq4}` 按开奖日期每日从 `1` / `01` / `001` / `0001` 递增。
- 期号生成默认使用彩种 `saleCloseLeadSeconds` 计算 `saleClosedAt = issueOpenedAt + saleCloseLeadSeconds`；超过计划开奖时间时按计划开奖时间封盘。后台手动生成接口显式传入 `saleCloseLeadSeconds` 时可以临时覆盖本次计算。
...
- 封盘时间必须使用彩种配置 `saleCloseLeadSeconds`；该值表示开盘后可售秒数，并与常驻调度保持一致。
