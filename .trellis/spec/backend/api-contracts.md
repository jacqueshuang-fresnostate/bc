...
- 彩种对象必须返回 `issueFormat`，默认 `{yyyy}{MM}{dd}{HH}{mm}{ss}`。
- 平台开奖生成期号时按 `issueFormat` 渲染计划开奖时间。
- API 开奖有开奖源锚点时必须使用外部最新期号顺延，不能使用 `issueFormat`。
- 支持变量：`{yyyy}`、`{yy}`、`{MM}`、`{dd}`、`{HH}`、`{mm}`、`{ss}`、`{date}`、`{time}`、`{timestamp}`。
- 保存彩种时需要校验模板至少包含一个时间变量，生成结果只能包含字母、数字、短横线或下划线，且不能超过 64 个字符。
