...
- 彩种对象必须返回 `saleCloseLeadSeconds`，默认 `1`，表示生成新期号时计划开奖时间前多少秒封盘。
- 平台开奖生成期号时按 `issueFormat` 渲染计划开奖时间；`{seq1}`、`{seq2}`、`{seq3}`、`{seq4}` 按开奖日期每日从 `1` / `01` / `001` / `0001` 递增。
- 期号生成默认使用彩种 `saleCloseLeadSeconds` 计算 `saleClosedAt`；后台手动生成接口显式传入 `saleCloseLeadSeconds` 时可以临时覆盖本次计算。
- 常驻调度、API 彩种未来期补齐、手动同步开奖源和开售后补期必须使用彩种自己的 `saleCloseLeadSeconds`，不能继续只读取调度器全局封盘提前秒数。
