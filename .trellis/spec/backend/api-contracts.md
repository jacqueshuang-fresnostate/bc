...
- 时间节点本身同时代表上一期的开奖点和下一期的开盘点；例如 `startTime=00:00:00` 且 `intervalSeconds=300` 时，`00:00:00` 是前一天最后一注的开奖时间，同时也是当天第一期的开盘时间，当天第一期在 `00:05:00` 开奖。
- 生成下一期时，开奖时间取严格大于基线时间的下一个自然节点；期号模板和每日 `{seqN}` 序号按本期开奖的开盘节点归属，不按开奖节点归属，避免 `00:00:00` 开奖被错误算成当天第一期。
- 如果存在历史偏移期号，例如 `20:18:27`，后续新期号仍回到最近的自然节点，例如 `20:20:00`，不再沿用偏移秒数。
- 手机端和后台展示中，`timeNode` 仍视为周期类彩种，可以返回和展示周期秒数，但文案需要标明“时间节点”以区别相对周期。

### 4. 必要测试

- 后端需要覆盖 `00:00:00 + 300 秒` 生成 `00:05:00` 的开奖时间。
- 后端需要覆盖跨零点边界：`23:55:00` 开盘、次日 `00:00:00` 开奖的期号仍归属前一天；`00:00:00` 开盘、`00:05:00` 开奖的期号才归属当天第一期。
...
- 未来期号缓冲统计同彩种、`status=open` 或 `status=closed` 且 `scheduledAt > now` 的期号；已封盘但未开奖的当前期也占用缓冲，避免封盘后提前开下一期。
- 需要补期时，补齐数量为 `futureIssueCount - existingFuturePendingIssueCount`。
...
| 未来待处理期号数量已达到 `futureIssueCount` | 不重复生成期号 |
...
- Bad：封盘后立即补出下一期 `open`，导致同一彩种在上一期未开奖时提前进入下一期销售。
...
- 范围：`LotteryKind.issueFormat`、`LotteryKind.saleCloseLeadSeconds`、`lotteries.issue_format`、`lotteries.sale_close_lead_seconds`、开奖期号生成器、后台彩种管理表单。
- 适用开奖模式：`platform`。
...
- 彩种对象必须返回 `saleCloseLeadSeconds`，默认 `1`，表示开奖前提前多少秒封盘。
- 平台开奖生成期号时按 `issueFormat` 渲染计划开奖时间；`{seq1}`、`{seq2}`、`{seq3}`、`{seq4}` 按开奖日期每日从 `1` / `01` / `001` / `0001` 递增。
- 期号生成默认使用彩种 `saleCloseLeadSeconds` 计算 `saleClosedAt = scheduledAt - saleCloseLeadSeconds`；例如 300 秒周期配置 60 秒时，在周期进行到 240 秒封盘并等待开奖。后台手动生成接口显式传入 `saleCloseLeadSeconds` 时可以临时覆盖本次计算。
- 常驻调度、API 彩种未来期补齐、手动同步开奖源和开售后补期必须使用彩种自己的 `saleCloseLeadSeconds`，不能继续只读取调度器全局封盘时间。
- API 开奖有开奖源锚点时必须使用外部最新期号顺延，不能使用 `issueFormat`。
- 支持变量：`{yyyy}`、`{yy}`、`{MM}`、`{dd}`、`{HH}`、`{mm}`、`{ss}`、`{date}`、`{time}`、`{timestamp}`、`{seq1}`、`{seq2}`、`{seq3}`、`{seq4}`。
- 保存彩种时需要校验模板至少包含一个日期或时间变量，生成结果只能包含字母、数字、短横线或下划线，且不能超过 64 个字符。
- 保存彩种时需要校验 `saleCloseLeadSeconds > 0`，数据库字段也必须保留同样约束。

### 3. 必要测试

- 后端需要覆盖平台开奖使用自定义 `issueFormat` 生成期号。
- 后端需要覆盖彩种 `saleCloseLeadSeconds` 会影响封盘时间，并覆盖请求显式传值可以临时覆盖彩种配置。
...
- 聊天大厅红包领取记录：`GET /api/user/chat-hall/red-packets/{id}/claims`
...
- `view=groupBuy` 分页前必须把 `groupBuyPendingPlan=true` 且未取消的“合买认购中”记录排在已成单、已结算等历史合买前面，再按创建时间倒序；否则用户刷新第一页时可能看不到仍在认购中的合买，只能加载更多后才出现。
- 分页必须在 `view` 过滤和对应视图排序之后执行，保证“我的注单”和“我的合买”两个 Tab 各自拥有稳定页码。
...
- `bb-hn5`：`hn5` 河内5分彩默认使用 BB 开奖接口，`gameCodeList=VIFFC5`，响应中按 `value[].last.numero` 作为最近已开奖锚点、`value[].newest.numero` 作为下一期参考；按期号开奖时从 `last/newest/next` 中查找对应 `numero`，只有存在非空 `openNumber` 才返回开奖号码。
- `indonesia-id5`：`id5` 印尼5分彩默认使用印尼开奖接口，响应中按 `latest_origin` 作为最近已开奖锚点、`next_num` 和 `next_time` 作为下一期参考；按期号开奖时把 `20260617-042`、`20260617-42` 归一为 `20260617042`，再从 `latest_num` 或 `history[].result` 读取开奖号码。
...
BB 开奖来源的 `lotCode` 字段在当前跨层模型中复用为 `gameCodeList`，例如河内5分彩保存 `lotCode="VIFFC5"`；后端请求时会按供应商自动拼接 `gameCodeList=VIFFC5`。后台展示标签可写为 `lotCode / lotKey / gameCode`。
印尼开奖来源的 `lotCode` 字段仅作为后台展示编码保存，例如印尼5分彩保存 `lotCode="IDFFC5"`；后端请求时直接使用 endpoint，不额外拼接查询参数。
...
4. 时间节点周期开奖：从 `startTime` 开始，按 `intervalSeconds` 找到严格晚于基线的下一个自然时间节点，例如 `00:00:00 + 300 秒` 生成 `00:05:00`、`00:10:00`；节点同时是上一期开奖点和下一期开盘点，期号按开盘节点渲染。
...
| `saleCloseLeadSeconds=0` | HTTP 400，返回 `封盘时间必须大于 0 秒` |
...
| `draw_scheduler_config.sale_close_lead_seconds=0` | 启动时配置校验失败，返回封盘时间错误 |
...
- Good：已有到期开奖期号时，单轮调度先完成开奖和结算，再补齐下一期并推送开盘。
- Good：已有期号刚到封盘时间但未到开奖时间时，单轮调度只把当前期转为 `closed` 并等待开奖时间，不生成下一期 `open`。
...
- Bad：未到开奖时间就因为当前期 `closed` 而补出下一期 `open`；这会让同一彩种提前进入下一期销售。
...
- 触发条件：新增或修改合买机器人、补单机器人配置管理接口，或新增机器人执行能力、调度器机器人编排、dashboard 机器人摘要。
...
7. `groupBuy` 是合买发单机器人，只负责把合买计划发到合买大厅；它不再补用户单，也不再补自己的单。
8. `purchase` 是补单机器人，只负责认购同彩种当前期未满单合买计划；后台展示名必须使用“补单机器人”，不要再展示为“购彩机器人”。
9. `groupBuyFillStrategy` 只允许 `rhythm` 或 `beforeDraw`，旧请求未传时默认 `rhythm`；该字段名沿用历史接口，但只对 `kind=purchase` 的补单机器人生效。
10. `groupBuyFillBeforeDrawSeconds` 表示开奖前多少秒触发补满，范围为 1 到 86400；`beforeDraw` 用它判断一次性补满窗口，`rhythm` 用它判断最终补满阈值。
11. `groupBuyRhythmFillMaxPercent` 表示阶段性补单单阶段随机补单上限百分比，默认 `20`，范围为 1 到 100；只有补单机器人且 `groupBuyFillStrategy=rhythm` 时参与执行判断。
12. `groupBuyRhythmStageCount` 表示阶段性补单动态切分阶段数量，默认 `3`，范围为 1 到 20；只有补单机器人且 `groupBuyFillStrategy=rhythm` 时参与执行判断，不包含开奖前最终补满阶段。
13. `POST /api/admin/robots/run` 同时执行已启用的 `groupBuy` 发单机器人和 `purchase` 补单机器人。
14. 机器人执行结果字段：
...
15. 合买机器人计划 ID 必须按“机器人 ID + 彩种 ID + 期号”确定性生成，同一期重复执行不能重复创建计划。
16. 合买机器人必须使用当前合买链路：校验彩种开售、合买开启、open 期号、封盘时间、启用玩法和注数报价，再创建计划并扣除发起人自购金额；存在计划时只记录跳过原因，等待补单机器人认购。
17. 合买机器人发单不能固定使用第一个玩法或固定样例号码；后端需要按机器人、彩种、期号确定性随机打散启用玩法，并按玩法随机选号数量，真实注数必须通过订单报价计算；隐含倍数通过合买总额换算为单注金额体现，单注金额必须能被手机端按 `200` 分基础金额换算为整数倍数。
18. 机器人系统账户集合为 `U90001`、`X90002`-`X90010`；发单机器人使用 `U90001`，补单机器人在 `rhythm` 阶段性补单时按阶段轮换该集合中的账号，在 `beforeDraw` 开奖前补满时只使用一个机器人账号一次性认购剩余金额。发起计划或补单前当前机器人账号余额不足时，后端必须先通过 `ManualAdjustment` 自动授信补余额并记录“机器人账户自动授信补余额”，随后再执行真实扣款，不允许出现未扣款计划。
19. 补单机器人必须扫描同彩种当前期所有 `draft/open` 未满单计划，包括用户、后台和合买机器人发起的大厅计划；满单后统一复用合买成单链路生成真实投注订单。
20. `rhythm` 策略从合买计划创建后到“开奖时间 - groupBuyFillBeforeDrawSeconds”之间按 `groupBuyRhythmStageCount` 动态切分阶段；例如还剩 260 秒开奖、最终补满阈值为开奖前 40 秒、阶段数量为 3 时，非最终阶段覆盖 260 秒到 40 秒之间的 3 个动态阶段。
21. `rhythm` 策略每个非最终阶段按计划和阶段稳定随机生成 1 到 `groupBuyRhythmFillMaxPercent` 之间的百分比，再折算为本阶段认购金额；同一计划同一阶段重复执行时必须跳过，不能因为机器人调度周期较短而重复补多笔上限金额。
22. `rhythm` 策略在距离开奖时间小于等于 `groupBuyFillBeforeDrawSeconds` 时直接补满剩余金额并触发满单成单；非最终阶段如果本轮补单会直接满单，或会让剩余金额低于参与最低认购额，必须等待最终补满。
23. `beforeDraw` 策略不按阶段追加认购，而是在距离开奖时间小于等于 `groupBuyFillBeforeDrawSeconds` 时一次性补满剩余金额；补满时只能新增一条机器人参与记录，不能把剩余金额拆成多条补单；如果当前时间已经超过开奖时间则跳过。
24. 补单机器人每次补单都必须生成新的参与记录 ID，不允许复用同一个机器人参与记录一次性覆盖剩余金额；写入参与记录前必须为每个机器人账号生成随机中文展示名，真实 `userId` 保持稳定。
25. 阶段性补单金额充足时，多用户拆分按稳定随机权重分配，不能完全均分导致同阶段机器人参与金额全部相同；金额不足时仍以参与最低金额和最低份额约束优先。
26. 合买发单机器人创建计划后的初始种子认购只允许执行一次，后续重复扫描已有未满单计划时不能继续追加种子认购。
27. 用户端合买计划响应必须隐藏 `G-ROBOT-` 开头的机器人计划真实发起人和机器人标题：`initiatorDisplay` 返回按计划 ID 稳定生成的普通用户昵称式展示名，机器人认购参与人也使用同一类随机昵称，不得使用“某某会员”模板；`title` 返回“彩种 第期号期合买”，不得把 `U90001`、`X90002`-`X90010`、`agent_alpha`、`robot_fill_02`、机器人名称或“合买机器人”暴露给手机端；后台机器人执行结果和后台合买管理仍可保留真实配置用于运营排查。
...
| `groupBuyFillBeforeDrawSeconds` 为 0 或超过 86400 | HTTP 400，返回补单机器人开奖前补满秒数错误 |
| `groupBuyRhythmFillMaxPercent` 为 0 或超过 100 | HTTP 400，返回补单机器人阶段补单最高百分比错误 |
| `groupBuyRhythmStageCount` 为 0 或超过 20 | HTTP 400，返回补单机器人阶段数量错误 |
...
| 已创建合买且处于阶段性补单非最终阶段 | HTTP 200，按动态阶段和随机上限追加机器人参与记录；如果金额约束不允许则进入 `skippedItems` |
| 当前合买进度已达到本阶段目标 | HTTP 200，进入 `skippedItems`，等待下一阶段 |
| 手动执行时机器人余额不足 | 自动写入机器人授信流水，再继续创建计划或补单并扣款 |

### 5. Good / Base / Bad Cases

- Good：创建 `purchase` 补单机器人并绑定 `fc3d`、`ssc60`，响应按标准字段返回，dashboard 同步显示该机器人。
...
- Good：`ssc60` 开售且开启合买、存在未封盘 open 期号时，`POST /api/admin/robots/run` 可以由合买机器人创建确定性机器人合买计划；未进入补单窗口时补单机器人只记录跳过原因，不立即补满。
- Good：进入阶段性补单后，补单机器人每个非最终阶段按后台配置的阶段数量和单阶段随机百分比上限追加机器人参与记录，到达开奖前配置秒数后补满计划并生成真实投注订单。
- Good：用户、后台或合买机器人已经发起同彩种当前期未满单合买时，补单机器人同样按临近封盘阶段目标追加机器人参与记录，不一次性补足剩余金额。
- Good：补单机器人配置 `groupBuyFillStrategy=beforeDraw` 时，进入开奖前配置秒数窗口后只新增一个补单机器人参与记录并吃掉全部剩余金额，随后复用合买成单链路生成真实订单。
- Good：如果调度错过分阶段补单窗口，且合买计划已经封盘但尚未到开奖时间，调度慢阶段和后台手动自动化必须在流单退款前调用补单机器人兜底补满；兜底补满仍走机器人参与记录、自动授信、真实订单创建和 `groupBuyDebit` 流水，不允许绕过财务校验。
...
- Good：补单机器人使用 `X90002`-`X90010` 参与合买时，参与记录对外用户名为随机中文名，资金、清理和过滤仍按真实 `userId` 执行。
...
- Bad：用户端合买列表或详情直接返回 `agent_alpha`、`robot_fill_02`、`U90001`、`X90002`、机器人名称或包含“机器人”的标题。
- Bad：开奖时间已经到达或期号已经开奖后再补单成单；这属于事后下单风险，必须进入失败记录或流单退款，而不是强行创建订单。
...
- 后端需要覆盖补单机器人按后台 `groupBuyRhythmStageCount` 动态切分阶段、按 `groupBuyRhythmFillMaxPercent` 随机上限补单，并在 `groupBuyFillBeforeDrawSeconds` 最终阈值生成真实订单。
- 后端需要覆盖补单机器人可以按相同节奏补满非机器人发起的当前期未满单计划。
- 后端需要覆盖阶段性补单金额不超过后台配置上限，并覆盖金额充足时多用户拆分不会完全均分。
- 后端需要覆盖 `beforeDraw` 策略进入开奖前窗口后只新增一个机器人参与记录，分别覆盖机器人发起计划和用户发起计划。
- 后端需要覆盖 `X90002`-`X90010` 补单账号会被识别为系统机器人，并且对外参与记录不会暴露 `robot_fill_*` 种子用户名。
- 后端需要覆盖封盘后开奖前的流单退款前兜底补满：已 `closed` 期号中的用户合买计划可以在开奖前补满并生成真实合买订单，随后不进入流单退款。
...
- 浏览器验证需要进入“合买机器人”和“补单机器人”入口，确认真实页面显示且控制台无错误。
...
22. 后台 `GET /api/admin/group-buy/plans` 不传 `includeRobotData` 时等同于 `false`，默认过滤 `initiatorUserId` 为系统合买/补单机器人账户集合（`U90001`、`X90002`-`X90010`）的计划；传 `includeRobotData=true` 时才展示机器人发起计划。机器人作为参与人补单的普通用户发起计划不能被过滤掉。
...
- Good：后台合买管理和用户端合买大厅都按创建时间倒序展示，分页前已经完成排序，最新创建的计划优先出现在第一页。
...
- 后端需要覆盖仍使用期号排序的合买计划摘要列表和详情列表按期号倒序返回，并覆盖用户端合买大厅分页按创建时间倒序返回。
...
| `saleCloseLeadSeconds=0` | HTTP 400，返回封盘时间必须大于 0 秒 |
...
- 机器人数据口径：财务总览、资金账户和资金流水默认隐藏系统机器人账户数据（`U90001`、`X90002`-`X90010`），只有页面开关传入 `includeRobotData=true` 时才展示。
...
- 范围：`GET /api/user/invitations/summary`、`AccessRepository` 用户代理关系、`InviteRepository` 邀请记录、`FinanceRepository` 充值与返利流水、`WithdrawalRepository` 已通过提现记录、`OrderRepository` 普通注单、`GroupBuyRepository` 合买参与记录、手机端 `mobile/src/api/user.ts`。
...
```json
{
  ...
  "registeredAt": "2026-06-05 18:30:00",
...
| 直属用户没有已通过提现 | `totalWithdrawalMinor=0` |
...
- 查看红包领取记录：`GET /api/user/chat-hall/red-packets/{id}/claims`
...
| 红包已领完 | 返回业务错误，不入账 |
| 查看红包领取记录 | 返回领取进度和领取人列表，空领取列表也应成功返回 |
| 分享不存在的合买计划 | 返回业务错误 |
| 分享非本人参与的合买计划 | 返回业务错误 |
...
- Good：用户点击已领完、自己发送或自己已领取过的红包时，可以通过领取记录接口看到领取人、金额和时间。
...
- 触发条件：后台运营需要导出用户充值记录，或在测试、运营维护场景中清理充值、提现、资金流水、投注、合买计划列表历史记录。
- 范围：后台财务管理、订单管理、合买管理、充值仓储、提现仓储、订单仓储、合买仓储、计奖派奖历史。
- 记录清理只处理历史记录，不做余额冲正；除资金流水清理接口外，其它清理接口不得删除资金流水，也不得重置流水号。
...
| 清理资金流水时没有数据 | 返回 `deletedCount=0`，账户余额和下一流水序号保持不变 |
| 清理资金流水后继续调账 | 新资金流水 ID 继续递增，不复用已清理的旧编号 |
...
| 清理合买记录且存在未完成计划 | 自动保留未结算计划，继续删除已取消或已结算计划 |
| 清理合买记录且计划均已取消或已结算 | 删除合买计划和参与记录，返回删除计划数量 |

### 5. Good / Base / Bad Cases

- Good：运营先导出充值 CSV，再清理充值记录，用户余额和资金流水仍保持不变。
- Good：清理资金流水后，资金账户余额保持不变，流水列表为空，后续手动调账生成的流水编号继续递增。
...
- Bad：清理资金流水时重算余额或重置 `finance_runtime.next_sequence`，导致余额被误改或后续流水 ID 重复。
- Bad：清理提现待审核申请但不解冻资金，导致用户冻结余额无法追溯。
- Bad：清理后重置流水号，导致后续充值单、提现单或投注单 ID 重复。

### 6. 必要测试

- 后端需要覆盖资金流水清理保留余额和下一流水序号、充值清理保留流水号、提现待审核拒绝清理、投注待开奖拒绝清理、已结算投注清理同时移除结算批次。
...
- 封盘时间必须使用彩种配置 `saleCloseLeadSeconds`；该值表示开奖前封盘提前秒数，并与常驻调度保持一致。
...
- 独立机器人调度器才能执行：
  - 合买机器人和补单机器人。
- 常驻开奖调度不得再直接执行常规合买机器人和补单机器人，常规发单/补单必须由 `spawn_robot_scheduler` 独立后台任务按自己的开关和周期执行。
- 到期开奖阶段仍可执行 `force_fill_user_group_buy_plans_before_refund` 流单前兜底补满；该兜底只服务即将退款或开奖的保护链路，不负责常规发单节奏。
- 到期开奖阶段仍需发布余额、订单和开奖相关实时事件，但不得影响下一期生成；独立机器人调度器只能发布机器人相关余额和订单事件。
...
| 独立机器人调度器还在执行 | 快阶段和到期开奖阶段继续执行，机器人常规发单或补单由机器人调度器自己的周期控制 |
...
- 后端需要覆盖常规 `run_group_buy_robots` 只能由手动接口或独立机器人调度器触发，开奖调度完整单轮只合并流单前兜底机器人结果。
...
- 触发条件：后台需要让合买机器人自动发起合买计划，补单机器人自动认购未满单合买，但不希望机器人慢任务拖住开奖封盘、开盘和结算。
- 范围：`spawn_robot_scheduler`、`RobotSchedulerRepository`、`GET /api/admin/robot-scheduler/status`、`PUT /api/admin/robot-scheduler/config`、管理后台机器人配置页。

### 2. 契约

- 机器人独立调度器配置只包含 `enabled` 和 `intervalSeconds`，字段使用 `camelCase` 返回给前端。
- 默认关闭，后台显式启动后才会按配置周期执行常规 `run_group_buy_robots`。
- 手动 `/api/admin/robots/run` 仍保留，用于运营立即执行一次机器人任务；该手动入口不修改调度器开关。
- 机器人独立调度器执行成功后需要记录新增合买、补满合买、生成订单、资金流水和跳过明细数量。
- 机器人独立调度器必须发布机器人扣款余额变化和机器人生成订单事件；不能发布开奖、封盘或开盘事件。
- 开奖调度器不得读取或写入 `robot_scheduler_*` 表；机器人调度器也不得生成或封盘期号。
- `DRAW_SCHEDULER_*` 等开奖调度配置不再影响常规机器人执行频率。

### 3. 错误与边界

| 条件 | 预期行为 |
|------|----------|
| `enabled=false` | 后台任务只低频轮询配置，不执行机器人 |
| `intervalSeconds=0` | 返回业务错误“机器人调度周期必须大于 0 秒” |
| 单轮机器人执行失败 | 写入失败运行记录，下一轮继续按周期尝试 |
| 机器人任务执行超过周期 | 记录中文 warning，下一轮追赶执行但不影响开奖调度 |
| 管理后台刷新页面 | 机器人页面同时读取机器人配置和机器人调度状态 |

### 4. 必要测试

- 后端需要运行 `cargo fmt --check` 和 `cargo check`，确保新增仓储、路由和迁移可编译。
- 后台需要运行 `npm run build`，确认机器人调度状态卡和配置保存接口类型可编译。
- OpenAPI 核心路径测试需要包含 `/admin/robot-scheduler/status` 和 `/admin/robot-scheduler/config`。
