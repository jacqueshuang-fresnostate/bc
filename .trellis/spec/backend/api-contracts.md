...
11. `POST /api/admin/robots/run` 同时执行已启用的 `groupBuy` 发单机器人和 `purchase` 补单机器人。
12. 机器人执行结果字段：
...
13. 合买机器人计划 ID 必须按“机器人 ID + 彩种 ID + 期号”确定性生成，同一期重复执行不能重复创建计划。
14. 合买机器人必须使用当前合买链路：校验彩种开售、合买开启、open 期号、封盘时间、启用玩法和注数报价，再创建计划并扣除发起人自购金额；存在计划时只记录跳过原因，等待补单机器人认购。
15. 合买机器人发单不能固定使用第一个玩法或固定样例号码；后端需要按机器人、彩种、期号确定性随机打散启用玩法，并按玩法随机选号数量，真实注数必须通过订单报价计算；隐含倍数通过合买总额换算为单注金额体现，单注金额必须能被手机端按 `200` 分基础金额换算为整数倍数。
16. 机器人系统账户集合为 `U90001`、`X90002`-`X90010`；发单机器人使用 `U90001`，补单机器人在 `rhythm` 阶段性补单时按阶段轮换该集合中的账号，在 `beforeDraw` 开奖前补满时只使用一个机器人账号一次性认购剩余金额。发起计划或补单前当前机器人账号余额不足时，后端必须先通过 `ManualAdjustment` 自动授信补余额并记录“机器人账户自动授信补余额”，随后再执行真实扣款，不允许出现未扣款计划。
17. 补单机器人必须扫描同彩种当前期所有 `draft/open` 未满单计划，包括用户、后台和合买机器人发起的大厅计划；满单后统一复用合买成单链路生成真实投注订单。
18. `rhythm` 策略使用封盘前 90 秒窗口：距离封盘 90-61 秒目标进度 40%，60-31 秒目标进度 60%，30-16 秒目标进度 80%，最后 15 秒才允许补到 100% 并触发满单成单；封盘点到开奖前仍保留兜底补满能力。
19. `beforeDraw` 策略不按阶段追加认购，而是在距离开奖时间小于等于 `groupBuyFillBeforeDrawSeconds` 时一次性补满剩余金额；补满时只能新增一条机器人参与记录，不能把剩余金额拆成多条补单；如果当前时间已经超过开奖时间则跳过。
20. 补单机器人每次补单都必须生成新的参与记录 ID，不允许复用同一个机器人参与记录一次性覆盖剩余金额；写入参与记录前必须为每个机器人账号生成随机中文展示名，真实 `userId` 保持稳定。
21. 用户端合买计划响应必须隐藏 `G-ROBOT-` 开头的机器人计划真实发起人和机器人标题：`initiatorDisplay` 返回按计划 ID 稳定生成的普通用户昵称式展示名，机器人认购参与人也使用同一类随机昵称，不得使用“某某会员”模板；`title` 返回“彩种 第期号期合买”，不得把 `U90001`、`X90002`-`X90010`、`agent_alpha`、`robot_fill_02`、机器人名称或“合买机器人”暴露给手机端；后台机器人执行结果和后台合买管理仍可保留真实配置用于运营排查。
...
- Good：进入补单窗口后，补单机器人按 40%、60%、80%、100% 的阶段目标追加机器人参与记录，最后 15 秒才补满计划并生成真实投注订单。
...
- 后端需要覆盖补单机器人在封盘前 90 秒窗口内按 40%、60%、80%、100% 节奏补单，并在最终阶段生成真实订单。
