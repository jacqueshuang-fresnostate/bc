...
- 跨仓储写操作必须使用同一个数据库事务或同一个内存事务边界；普通下注等高频热路径优先使用 PostgreSQL 行级锁和增量写入，不能为了扣一笔款克隆全量快照或重写全量历史。
...
- 用户累计真实充值金额属于资格判断类数据，数据库模式必须读取 `user_withdrawal_turnovers.cumulative_recharge_minor`；聊天大厅发言门槛、代理中心累计充值等不能直接扫描 `ledger_entries` 审计流水，否则一键清除资金流水后会把已充值用户误判为未充值。
...
- `lotteries.sale_close_lead_seconds` 保存彩种开奖前封盘提前秒数，默认 `1` 且必须大于 `0`；生成期号时按“计划开奖时间 - 封盘提前秒数”计算 `sale_closed_at`，例如 300 秒周期配置 `60` 时，在周期进行到 240 秒封盘并等待开奖。字段名和业务含义保持一致。
...
期号创建成功后，如果彩种开启避奖且 Redis 启用，必须初始化该期所有合法候选号码，score 初始为 0，并设置 12 小时 TTL。下注订单最终保留后，必须按正式派奖公式计算每个命中候选的预计派奖金额并 `ZINCRBY`；取消待开奖真实订单时必须反向扣回。

批量 `ZINCRBY` 使用 Redis pipeline 时，返回值是每条命令累加后的新分数列表；调用方必须接收 `Vec<f64>` 或显式忽略每条命令返回，不能按空返回 `()` 解析，否则会把实际 Redis 响应误判为写入失败或让写分状态不可诊断。
...
批量 `ZINCRBY` 会返回新分数列表，按空返回解析会导致 Redis 响应处理不正确。

#### Correct

```rust
let _: Vec<f64> = pipe.query_async(&mut connection).await?;
```

接收 Redis 返回的新分数列表，让写分结果和错误状态保持可诊断。
...
- 事务内增量保存：订单、资金、合买、充值、提现等仍使用快照协调的热路径使用 `save_*_store_incremental_in_transaction(connection, previous, store)`
- 普通下注数据库事务：`OrderRepository::create_many_with_debit_in_database(finance, requests, OrderSource::Direct)`
- Redis 投注锁：用户端普通下注路由按 `lock:user:<用户ID>:bet` 获取短时分布式锁，Redis 未配置时跳过
...
- Bad：普通下注为了扣一笔款调用 `save_finance_store_incremental_in_transaction` 并锁资金三张表；用户量上来后会把不同用户下注互相堵住。
- Bad：路由先调用 `orders.create()` 再调用 `finance.debit_order()`；这会在中途失败时留下半边状态。

### 6. 必要测试

- 后端需要覆盖批量下注失败不产生部分订单和扣款。
- PostgreSQL 下注热路径变更时需要覆盖同一用户串行扣款、余额不足回滚和流水序号不重复。
...
事务协调入口负责订单和资金一致性；PostgreSQL 普通下注会走行级事务和增量写入，内存模式或低频链路才使用快照协调。
...
| `api_draw_delay_seconds` | `integer not null default 0` | API 开奖源延迟秒数，仅 `draw_mode=api` 生效；调度在官方开奖时间后等待该秒数再请求第三方开奖号码 |
...
合买模块仍使用运行时快照保存时，所有创建、认购、回滚、满单关联订单和结算回写都必须经过仓储级写操作锁；满单创建真实投注订单和回写 `group_buy_plans.order_id` 必须在同一个订单合买事务中保存，不能先落 `orders` 再单独落合买计划。数据库保存前必须锁定 `group_buy_participants` 和 `group_buy_plans`，避免多个快照异步落库时旧数据覆盖新数据。参与记录写入失败时日志必须输出实际数据库错误、合买计划 ID、参与记录 ID、用户 ID、金额和份数，接口可以继续返回中文内部错误文案。
