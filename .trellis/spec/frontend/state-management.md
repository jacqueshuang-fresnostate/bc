...
- 手机端首页通过底部导航频繁进出，首页聚合数据、广告和余额不能在每次组件挂载时无条件重新请求。
- 首页服务端状态应放在 Pinia store 中缓存，页面只消费 store refs，并调用 store 的 `load*` 方法。
...
- 有缓存时不要显示整页 loading；过期刷新应静默进行，避免用户返回首页时看到重复加载闪烁。
