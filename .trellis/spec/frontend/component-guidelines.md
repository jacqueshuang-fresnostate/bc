...
- 手机端公共聊天大厅必须使用当前系统 `GET/POST /api/user/chat-hall/messages` 和 `chat_hall_message_created` 实时事件；大厅是所有登录用户的公共聊天流，不得复用或写入客服会话接口，断线重连后通过 HTTP 拉取最近历史并在 WS 事件到达时按消息 ID 去重追加。聊天大厅输入栏接入表情时沿用手机端客服的 `emoji-mart` 原生 `Picker`、动态加载、`Teleport` 面板、Vue 遮罩关闭和不传 `onClickOutside` 的规则，避免第二次打不开或实时消息重渲染错误。聊天大厅加入 `mobile-bottom-nav` 后，页面必须为底部导航预留空间，输入栏、表情面板和消息列表底部不能被导航遮挡。聊天大厅顶部只保留标题和说明文案，不展示返回或刷新图标按钮。
