...
- 手机端首页普通分类彩种卡片使用顶部名称/状态/期号与右侧彩种 logo、底部整宽开奖号码的纵向结构；状态标签和期号必须分行展示，避免状态标签挤占期号空间。开奖号码行必须脱离左侧文案区域并放在卡片底部，单行不换行，号码位数较多时允许号码行内部横向滑动，不要让号码换行撑高卡片。不要再用头像和号码上下堆叠撑高卡片。logo 容器需要固定宽高和圆角，缺失 logo 时显示同尺寸 fallback，保证两列布局高度稳定。
...
- 后台彩种新增/编辑表单在开奖模式选择“平台开奖”时必须显示“平台期号格式”输入项；默认 `{yyyy}{MM}{dd}{HH}{mm}{ss}`，提交字段为 `issueFormat`，只影响平台开奖期号生成，不能影响 API 开奖源期号同步。
