...
- 手机端下注页开启合买模式时，自购份数需要按方案总额、固定每份金额和发起人最低自购比例自动填入最适配份数：有最低比例时填最低所需份数，无最低比例时至少填 1 份；用户手动填更高份数时保留，但低于最低份数或超过总份数时必须自动校正。自购份数输入必须放在底部固定投注栏里，底栏仍沿用普通下注的左右两栏结构，左侧展示自购份数、共计注数、方案总额和实际需支付金额，右侧展示“投注”主按钮；不要做成和普通下注差异过大的独立结算卡片。
...
- 后台合买管理需要提供“一键清除合买计划列表”按钮；按钮在加载中、保存中或当前口径无记录时禁用，点击后必须用中文确认清理范围和风险。清理调用 `DELETE /api/admin/group-buy/plans/clear`，成功后关闭详情抽屉、回到第 1 页、刷新列表和 dashboard；后端拒绝未完成计划时直接展示接口错误，不做前端本地假删除。
