...
- 手机端入口、全局 CSS 或触摸工具变更后，需要确认 `meta viewport` 仍包含 `user-scalable=no`、`maximum-scale=1.0`，并确认 `installMobileTouchZoomGuard()` 仍在启动时执行，避免用户拖动、双指或双击导致页面整体缩放；同时要检查普通单指上下滚动、弹窗内部滚动和聊天消息列表滚动没有被触摸防护误拦截。
