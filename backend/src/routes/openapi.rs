...
const ROUTE_DOCS: &[RouteDoc] = &[
    ...
    doc(
        ...
        "合买管理",
        "强制清除机器人合买计划",
        "删除纯机器人合买计划，以及这些计划关联的机器人合买投注订单；不限制草稿、进行中、已满单、已取消或已结算状态，真实用户参与过的合买计划会保留。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        ...
        "邀请返利",
        "代理返利统计",
        "分页返回每个代理的直属下级数、直属下级已入账充值总额、直属下级已通过提现总额、返利总额、已提现返利、待处理返利和当前可处理金额。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        "post",
        "/admin/rebate-statistics/{agentUserId}/withdrawals",
        "邀请返利",
        "处理代理返利提现",
        "从代理可用余额中扣减指定返利金额，并写入代理返利提现资金流水。",
        AuthMode::Admin,
        RequestBodyKind::Json,
    ),
    doc(
    ...
    doc(
        ...
        "机器人配置",
        "机器人列表",
        "返回合买机器人和补单机器人配置，合买机器人只负责发单，补单机器人包含补单策略、阶段补单最高百分比、阶段数量与开奖前补满秒数。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
        ...
        "机器人配置",
        "新增机器人",
        "创建机器人配置，补单机器人可选择阶段性补单或开奖前补满策略，并维护阶段补单最高百分比、阶段数量与开奖前补满秒数。",
        AuthMode::Admin,
        RequestBodyKind::Json,
    ),
    doc(
    ...
    doc(
        ...
        "彩种管理",
        "彩种列表",
        "返回所有彩种、玩法赔率、分类、Logo、平台期号格式、开奖号码控制开关和避开中奖开关。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        "patch",
        "/admin/lotteries/{id}/avoid-winning",
        "开奖源与调度",
        "修改彩种避奖状态",
        "彩种控制台快速开启或关闭自动避开中奖策略。",
        AuthMode::Admin,
        RequestBodyKind::Json,
    ),
    doc(
    ...
    doc(
        "get",
        "/admin/draw-source-snapshots",
        "开奖源与调度",
        "API 采集快照",
        "分页返回第三方开奖源每次抓取的期号、开奖号码、成功状态和原始响应，支持按彩种、来源、用途、成功状态和期号筛选。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
        "delete",
        "/admin/draw-source-snapshots/clear",
        "开奖源与调度",
        "清除 API 采集快照",
        "一键清除第三方开奖源抓取快照审计记录；只删除快照，不影响开奖源配置、开奖期号、调度和开奖结果。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        ...
        "开奖源与调度",
        "保存控制号码",
        "保存彩种指定期号控制开奖号码配置；彩种未开启开奖号码控制时不能启用控制配置。",
        AuthMode::Admin,
        RequestBodyKind::Json,
    ),
    doc(
    ...
    doc(
        ...
        "订单与结算",
        "订单列表",
        "分页返回后台订单列表，包含订单期号、用户 ID、用户名和下注信息；默认过滤机器人账户订单，可通过 page、pageSize 和 includeRobotData=true 查询，彩种控制台不传分页参数时会复用该接口读取完整订单页查看用户下注信息。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        ...
        "订单与结算",
        "结算批次列表",
        "分页返回开奖结算批次列表，订单明细包含用户 ID 和用户名，可通过 page 和 pageSize 查询。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
        ...
        "订单与结算",
        "结算批次详情",
        "按结算批次 ID 返回详情，订单明细包含用户 ID 和用户名。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
        ...
        "订单与结算",
        "结算指定期号",
        "对已开奖期号执行订单结算和派奖，返回的订单明细包含用户 ID 和用户名。",
        AuthMode::Admin,
        RequestBodyKind::None,
    ),
    doc(
        ...
        "用户端内容",
        "手机端首页彩种",
        "返回所有销售中的彩种，按后台彩种分类分组，并附带当前期号、最近开奖号码和后台配置的高频极速推荐区。",
        AuthMode::None,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        ...
        "用户端内容",
        "手机端站点配置",
        "返回手机端平台名称、Logo 图片链接、默认头像图片链接和站点介绍。",
        AuthMode::None,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        ...
        "用户端账户",
        "用户注册",
        "支持用户名注册或邮箱注册，注册时必须提交 5-12 位数字 QQ，并按策略处理代理邀请码；Android、iOS 和 H5 可提交 registrationPosition 经纬度，后端统一用 Nominatim 反查中文注册地。",
        AuthMode::None,
        RequestBodyKind::Json,
    ),
    doc(
    ...
    doc(
        ...
        "用户端账户",
        "邀请中心汇总",
        "返回当前用户邀请码、邀请权限、直属用户、直属充值汇总、真实已结算返利和当前返利策略摘要。",
        AuthMode::User,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        ...
        "用户端合买",
        "合买大厅列表",
        "返回当前用户可查看的合买计划、发起人脱敏名称和普通发起人头像，支持按彩种和分组筛选。",
        AuthMode::User,
        RequestBodyKind::None,
    ),
    doc(
    ...
    doc(
        ...
        "用户端客服",
        "用户发送客服消息",
        "当前用户向自己的客服会话追加文本或图片消息；图片消息需要先通过客服图片上传接口获取 imageUrl。",
        AuthMode::User,
        RequestBodyKind::Json,
    ),
    doc(
        "post",
        "/user/support/images/upload",
        "用户端客服",
        "上传客服图片",
        "当前用户上传客服聊天图片，后端按系统图床配置代理上传并返回 imageUrl。",
        AuthMode::User,
        RequestBodyKind::Multipart,
    ),
    doc(
